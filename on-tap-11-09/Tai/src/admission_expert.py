"""Bài 2: hệ luật tư vấn tuyển sinh, mạng ngữ nghĩa và kiểm tra mâu thuẫn."""
...
def build_rules() -> List[Rule]:
    return [
        Rule("R01", "Toán >= 8.0 VÀ Anh >= 7.5", "Khoa học máy tính", lambda c: c.math >= 8 and c.english >= 7.5),
        Rule("R02", "Tin >= 8.5", "Khoa học máy tính", lambda c: c.informatics >= 8.5),
        Rule("R03", "Toán >= 7.0 VÀ (Tin >= 7.0 HOẶC Vật lý >= 7.5)", "Kỹ thuật phần mềm", lambda c: c.math >= 7 and (c.informatics >= 7 or c.physics >= 7.5)),
        Rule("R04", "Toán >= 7.5 VÀ Vật lý >= 7.5 VÀ Hóa >= 6.5", "Kỹ thuật điện tử", lambda c: c.math >= 7.5 and c.physics >= 7.5 and c.chemistry >= 6.5),
        Rule("R05", "Sinh >= 8.0 VÀ Hóa >= 7.0", "Công nghệ sinh học", lambda c: c.biology >= 8 and c.chemistry >= 7),
        Rule("R06", "Sinh >= 7.0 VÀ Hóa >= 7.5 VÀ sở thích y-sinh", "Khoa học sức khỏe", lambda c: c.biology >= 7 and c.chemistry >= 7.5 and "y-sinh" in c.interests),
        Rule("R07", "Văn >= 7.5 VÀ Anh >= 7.0 VÀ sở thích truyền thông", "Truyền thông đa phương tiện", lambda c: c.literature >= 7.5 and c.english >= 7 and "truyền thông" in c.interests),
        Rule("R08", "Toán >= 7.0 VÀ Anh >= 7.0 VÀ quan tâm kinh doanh", "Quản trị kinh doanh", lambda c: c.math >= 7 and c.english >= 7 and "kinh doanh" in c.interests),
        Rule("R09", "Văn >= 8.0 VÀ Anh >= 7.0 VÀ định hướng luật", "Luật", lambda c: c.literature >= 8 and c.english >= 7 and c.career == "luật"),
        ...
        Rule("R11", "Anh >= 8.0 VÀ Văn >= 7.0 VÀ sở thích ngôn ngữ", "Ngôn ngữ Anh", lambda c: c.english >= 8 and c.literature >= 7 and "ngôn ngữ" in c.interests),
...
def infer(candidate: Candidate, rules: List[Rule] | None = None) -> List[Rule]:
    return [rule for rule in (rules or build_rules()) if rule.predicate(candidate)]
...
def build_llm_context(candidate: Candidate) -> str:
    rules_text = "\n".join(f"- {r.code}: NẾU {r.condition_text} THÌ phù hợp {r.recommendation}." for r in build_rules()[:-1])
    return (
